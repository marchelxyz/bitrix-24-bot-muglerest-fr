...
if DATABASE_AVAILABLE:
    try:
        ...
        # Загружаем маппинги thread_to_department из БД
        db_thread_mappings = database.get_all_thread_to_department_mappings()
        if db_thread_mappings:
            THREAD_TO_DEPARTMENT_MAPPING.update(db_thread_mappings)
            logger.info(f"✅ Загружено {len(db_thread_mappings)} маппингов thread_id -> department_id из БД")
    except Exception as e:
        logger.error(f"❌ Ошибка при инициализации PostgreSQL: {e}", exc_info=True)
        logger.warning("⚠️ Бот будет работать с локальным хранилищем (в памяти)")
        DATABASE_AVAILABLE = False
else:
    logger.warning("⚠️ PostgreSQL недоступен. Используется локальное хранилище (в памяти)")

# Загружаем существующие связи из Bitrix24 при старте (для миграции/синхронизации)
...
try:
    ...
    if thread_mapping_str:
        ...
        thread_mappings = {int(k): int(v) for k, v in thread_mapping_dict.items()}
        THREAD_TO_DEPARTMENT_MAPPING.update(thread_mappings)
        
        # Сохраняем в БД если доступна
        if DATABASE_AVAILABLE:
            for thread_id, dept_id in thread_mappings.items():
                database.set_thread_to_department_mapping(thread_id, dept_id)
            logger.info(f"✅ Загружено и сохранено в БД {len(thread_mappings)} маппингов thread_id -> department_id из переменной окружения")
        else:
            logger.info(f"✅ Загружено {len(thread_mappings)} маппингов thread_id -> department_id (в память)")
    else:
...
async def log_telegram_group_info(application: Application):
    """
...
def format_deadline_for_display(deadline: str) -> str:
    ...
    Args:
        deadline: Дедлайн в формате YYYY-MM-DD HH:MM:SS (МСК время, которое передается как есть)
    ...
    try:
        ...
        # Время уже в МСК формате, передаем как есть
        deadline_dt = datetime.strptime(deadline, "%Y-%m-%d %H:%M:%S")
        # Возвращаем в том же формате для отображения (уже в МСК)
    ...
    try:
        ...
        if deadline_text.startswith('через'):
            ...
            if days is not None:
                ...
                # Передаем время как есть, без конвертаций - Bitrix интерпретирует его как МСК
                if deadline_date.tzinfo:
                    deadline_date = deadline_date.replace(tzinfo=None)
                # Форматируем дату (в формате МСК, который Bitrix интерпретирует как МСК)
...
def get_bitrix_user_id_by_telegram_id(telegram_id: int) -> Optional[int]:
    ...
    1. PostgreSQL база данных (если доступна)
    2. Локальное хранилище (память) - fallback
    3. Bitrix24 API - прямой запрос к API
    ...
    """
    # Сначала проверяем PostgreSQL БД
    if DATABASE_AVAILABLE:
        try:
            bitrix_id = database.get_bitrix_user_id_by_telegram_id(telegram_id)
            if bitrix_id:
                # Обновляем кеш в памяти для быстрого доступа
                TELEGRAM_TO_BITRIX_MAPPING[telegram_id] = bitrix_id
                return bitrix_id
        except Exception as e:
            logger.debug(f"Ошибка при запросе к БД для telegram_id {telegram_id}: {e}")
    
    # Если не найдено в БД, проверяем локальное хранилище (кеш)
    bitrix_id = TELEGRAM_TO_BITRIX_MAPPING.get(telegram_id)
    if bitrix_id:
        return bitrix_id
    
    # Если не найдено в кеше, запрашиваем через Bitrix24 API
    try:
...
async def create_task_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.message_thread_id:
        ...
        # Автоматически определяем отдел на основе thread_id (из БД или памяти)
        if DATABASE_AVAILABLE:
            try:
                department_id = database.get_department_id_by_thread_id(thread_id)
            except Exception:
...
async def link_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Пробуем извлечь ID из текста после /link
    # Поддерживаем форматы: /link 123, /link123, /link@bot 123, /link@bot123
    match = re.search(r'^/link(@\w+)?\s*(\d+)', text)
    if match:
        bitrix_id_str = match.group(2)
        logger.info(f"🔍 Извлечен Bitrix ID через regex: '{bitrix_id_str}'")
    elif context.args and len(context.args) >= 1:
        # Если есть аргументы в context.args (для CommandHandler), используем их
        bitrix_id_str = context.args[0]
        logger.info(f"🔍 Использован context.args[0]: '{bitrix_id_str}'")
    else:
        # Пробуем разбить текст по пробелам
        parts = text.split(None, 1)
        if len(parts) >= 2:
            # Удаляем имя бота если есть
            second_part = re.sub(r'^@\w+\s*', '', parts[1])
            if second_part.isdigit():
                bitrix_id_str = second_part
                logger.info(f"🔍 Извлечен из parts[1]: '{bitrix_id_str}'")
            else:
    ...
    try:
        ...
        if DATABASE_AVAILABLE:
            ...
            if db_success:
                ...
                logger.warning(f"⚠️ Не удалось сохранить связь в PostgreSQL (детали ошибки в логах выше)")
        ...
        if db_success or DATABASE_AVAILABLE == False:
            ...
            if db_success:
                response_text += f"✅ Связь сохранена в PostgreSQL\n\n"
            elif DATABASE_AVAILABLE:
                response_text += (
                    f"⚠️ Связь сохранена в локальном хранилище (PostgreSQL недоступен)\n"
                    f"💡 Проверьте логи для деталей ошибки PostgreSQL\n\n"
                )
            else:
                response_text += f"ℹ️ Связь сохранена в локальном хранилище (PostgreSQL не настроен)\n\n"
            
            if bitrix_success:
                response_text += f"✅ Связь также синхронизирована с Bitrix24\n\n"
            else:
                response_text += (
                    f"⚠️ Не удалось синхронизировать с Bitrix24\n\n"
                    f"Возможные причины:\n"
            ...
            response_text = (
                f"⚠️ Связь установлена в локальном хранилище:\n"
                f"Ваш Telegram аккаунт (ID: {telegram_user_id}) → "
                f"{user_name} (ID: {bitrix_user_id})\n\n"
                f"❌ Не удалось сохранить связь в PostgreSQL.\n\n"
...
async def check_telegram_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Проверяем наличие Telegram ID в Bitrix24
        telegram_id = bitrix_client.get_user_telegram_id(bitrix_user_id)
        
        # Также проверяем в локальном кеше
        if not telegram_id:
            for tg_id, bx_id in TELEGRAM_TO_BITRIX_MAPPING.items():
                if bx_id == bitrix_user_id:
                    telegram_id = tg_id
                    break
        ...
        if telegram_id:
            ...
            response_text += f"Связь найдена в Bitrix24!"
            
            # Проверяем, есть ли в кеше
            if telegram_id in TELEGRAM_TO_BITRIX_MAPPING:
                response_text += "\n💡 Связь также загружена в память бота (кеш)."
        else:
            response_text += f"❌ Не найдено\n\n"
            response_text += (
                f"Telegram ID не найден в Bitrix24.\n\n"
                f"Используйте команду /link bitrix_user_id для связывания аккаунта.\n\n"
                f"💡 Если Telegram ID сохранен в Bitrix24, но не отображается здесь, "
                f"это может означать, что:\n"
                f"1. Поле '{bitrix_client.telegram_field_name}' не возвращается в API\n"
                f"2. Исходящий вебхук не настроен для синхронизации\n"
                f"3. Данные еще обрабатываются Bitrix24"
            )
...
async def link_username(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Сохраняем в PostgreSQL БД и локальное хранилище
        if DATABASE_AVAILABLE:
            database.set_username_to_bitrix_mapping(telegram_username, bitrix_user_id)
        ...
        await update.message.reply_text(
            ...
            f"💡 Связь username хранится только в памяти и будет потеряна при перезапуске бота.\n"
            f"Для постоянного хранения используйте команду /link с Telegram ID."
        )
    except ValueError:
        await update.message.reply_text("❌ ID пользователя должен быть числом")
...
async def start_task_creation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not creator_id:
        await update.message.reply_text(
            ...
            "После связывания связь будет сохранена в Bitrix24, "
            "и бот будет автоматически определять ваш аккаунт.\n\n"
            "💡 Рекомендуется настроить исходящий вебхук в Bitrix24 для автоматической "
            "синхронизации данных при обновлении пользователей."
        )
        return ConversationHandler.END
...
def main():
    ...
    application.add_handler(MessageHandler(
        filters.TEXT & filters.Regex(r'^/link(@\w+)?(\s+\d+|\d+|$)'),
        link_user
    ))
    application.add_handler(MessageHandler(
    ...
    if port > 0:
            ...
            try:
                ...
                async def post_init(aio_app):
                    ...
                    try:
                        ...
                        # Выводим информацию о супергруппе и разделах в консоль
                        await log_telegram_group_info(application)
                        ...
                        if 'USERUPDATE' in event.upper() or 'USERADD' in event.upper() or (event.upper().startswith('ONUSER') and 'USER' in event.upper()):
                            ...
                            if telegram_id_str:
                                try:
                                    ...
                                    # Проверяем, не изменилась ли связь в кеше/БД
                                    existing_bitrix_id = None
                                    if DATABASE_AVAILABLE:
                                        try:
                                            existing_bitrix_id = database.get_bitrix_user_id_by_telegram_id(telegram_id)
                                        except Exception:
                                            existing_bitrix_id = TELEGRAM_TO_BITRIX_MAPPING.get(telegram_id)
                                    else:
                                        existing_bitrix_id = TELEGRAM_TO_BITRIX_MAPPING.get(telegram_id)
                                    ...
                                    logger.info(
                                        f"✅ Синхронизировано из Bitrix24 (исходящий вебхук): "
                                        f"Telegram ID {telegram_id} → Bitrix24 User ID {user_id_int}"
                                    )
                                except (ValueError, TypeError) as e:
        ...
        async def post_init_polling(app: Application):
            ...
            # Выводим информацию о супергруппе и разделах в консоль
            await log_telegram_group_info(app)
